...
while True:
    try:
        qpca30 = qPCA(svd_solver='full', name='qpca30').fit(trains[0].drop(columns='41'), theta_estimate=True,
                                                            ...
                                                            p=0.30,
                                                            estimate_all=True,
                                                            delta=0.1, eps=5, true_tomography=True, eta=0.2)
    except:
        pass
    else:
        break
...
while True:
    try:
        qpca60 = qPCA(svd_solver='full', name='qpca60').fit(trains[3].drop(columns='41'), theta_estimate=True,
                                                            ...
                                                            p=0.60,
                                                            estimate_all=True,
                                                            delta=0.9, eps=3, true_tomography=True,eta=0.3)
    except:
        pass
    else:
        break
...
while True:
    try:
        qpca70 = qPCA(svd_solver='full', name='qpca70').fit(trains[4].drop(columns='41'), theta_estimate=True,
                                                            ...
                                                            p=0.70,
                                                            estimate_all=True,
                                                            delta=0.1, eps=3, true_tomography=True,eta=0.2)
    except:
        pass
    else:
        break
...
qmodel = Model(QPCAs, quantils, quantum=True).fit(trains_without_labels, minor_sv_variance=0.20, only_dot_product=True,experiment=0)
...
qpca70.runtime_comparison(10000, 10, 'KDDUCUPExp0_1zoom.pdf', estimate_components='right_sv', classic_runtime='rand')
'''dictionary_major = {}
dictionary_minor = {}
